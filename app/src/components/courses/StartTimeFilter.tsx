...
export function StartTimeFilter() {
  ...
  const endTimeMax = Route.useSearch({ select: (s) => s.endTimeMax })
  const navigate = Route.useNavigate()

  const isActive = startTimeMin !== undefined || endTimeMax !== undefined

  const sliderMin = startTimeMin !== undefined ? toMinutes(startTimeMin) : undefined
  const sliderMax = endTimeMax !== undefined ? toMinutes(endTimeMax) : undefined
    ...
    void navigate({
      search: (prev) =>
        ({
          ...
          endTimeMax: max !== undefined ? toIsoTime(max) : undefined,
          page: 1,
  ...
  return (
    <div className="flex flex-col gap-2.5">
      <div className="flex items-center gap-1">
        <span className="text-xs font-medium text-slate-500 uppercase">Class time</span>
        {isActive && (
          <button
            type="button"
            onClick={clearFilter}
            aria-label="Clear class time filter"
